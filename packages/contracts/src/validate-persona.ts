...
function expectObject(v: unknown, path: Path, errors: Issue[]): Obj | undefined {
...
function expectString(v: unknown, path: Path, errors: Issue[]): string | undefined {
  if (v === undefined) return undefined
 ...
 */
function expectStringArray(v: unknown, path: Path, errors: Issue[], check?: (s: string) => string | undefined): string[] | undefined {
  if (v === undefined) return undefined
  if (!Array.isArray(v)) {
...
function unknownFields(o: Obj, known: readonly string[], path: Path, errors: Issue[]): void {
