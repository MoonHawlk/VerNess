...
function writePatch(cfg) {
  ...
  for (const [name, r] of Object.entries(routes)) {
    ...
    const ids = r.kind === 'local' ? localModels(cfg) : [...new Set([r.id, ...(r.models ?? [])].filter(x => x !== undefined))]
    for (const id of ids) {
      L.push(`              - id: ${yq(id)}`)
  ...
  // choice lives in .verness/state.json so switching never rewrites the commented config file.
...
async function cmdRun(cfg, task) {
  ...
  step(`VerNess ready - persona ${paint(C.bold, activePersonaId(cfg))}, model ${paint(C.bold, boot.model ?? '-')} via ${boot.name} (tools: ${accessMode()})`)
